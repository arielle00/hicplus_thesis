...
def train(lowres, highres, outModel):
    ...
    for i in range(high_resolution_samples.shape[0]):
        no_padding_sample = high_resolution_samples[i][0][half_padding:(sample_size-half_padding),
    ...
    with open('HindIII_train.txt', 'w') as log:
        for epoch in range(0, 3500):
            ...
            print('-------', i, epoch, avg_loss, strftime("%Y-%m-%d %H:%M:%S", gmtime()))
            log.write(f"{epoch}, {avg_loss}, {strftime('%Y-%m-%d %H:%M:%S', gmtime())}\n")
    ...
    print(f"Training finished. Best model was at epoch {best_epoch} with loss {best_loss:.4f}")
